...
/datum/artifact_effect/extreme/spines/proc/shoot(var/list/exempt = list())
	get_master_holdeR().visible_message("<span class='danger'>\The [get_master_holder()] fires spines wildly in all directions!</span>")
	for(var/i in range(1, round(chargelevel / 3))
		...
		P.launch_projectile(target, BP_TORSO, carrier)
		chargelevel -= 3
