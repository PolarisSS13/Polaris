...
/datum/event/lore_news/announce()
	...
	"Doctors in [pick(pick(skrell, heights))] were left in awe and horror as a   was hatched with the features of an adult human man! Sources say that the youngster hatched with a full head of hair and could recite six nursery rhymes from memory. Could Skrell genetic experiments be to blame?",
	...
	"Lead Singer of Strawberry Idol Revealed As Three   In A Trench Coat-- Newest gimmick or shocking revelation??",
