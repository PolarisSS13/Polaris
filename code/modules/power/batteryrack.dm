...
/obj/machinery/power/smes/batteryrack/Destroy()
	for(var/obj/item/stock_parts/cell/C in internal_cells)
		qdel(C)
	internal_cells = null
	return ..()
...
/obj/machinery/power/smes/batteryrack/add_charge(var/amount)
	...
	if(equalise)
		...
		var/obj/item/stock_parts/cell/CL = get_least_charged_cell()
...
/obj/machinery/power/smes/batteryrack/remove_charge(var/amount)
	...
	if(equalise)
		...
		var/obj/item/stock_parts/cell/CL = get_most_charged_cell()
...
/obj/machinery/power/smes/batteryrack/proc/get_most_charged_cell()
	var/obj/item/stock_parts/cell/CL = null
	for(var/obj/item/stock_parts/cell/C in internal_cells)
		if(CL == null)
			CL = C
...
/obj/machinery/power/smes/batteryrack/proc/insert_cell(var/obj/item/stock_parts/cell/C, var/mob/user)
	if(!istype(C))
		return 0
...
/obj/machinery/power/smes/batteryrack/process()
	...
	if(equalise)
		var/obj/item/stock_parts/cell/least = get_least_charged_cell()
		var/obj/item/stock_parts/cell/most = get_most_charged_cell()
...
/obj/machinery/power/smes/batteryrack/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if(default_part_replacement(user, W))
		return
	if(istype(W, /obj/item/stock_parts/cell)) // ID Card, try to insert it.
...
/obj/machinery/power/smes/batteryrack/Topic(href, href_list)
	...
	if( href_list["disable"] )
		...
		for(var/obj/item/stock_parts/cell/CL in internal_cells)
