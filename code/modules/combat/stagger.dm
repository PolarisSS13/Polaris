...
/mob/living/proc/stagger(var/mob/living/user = null, var/turf/T = null, var/strength = 1)
	...
	if(poise != 0 || !(poise < strength) ) //If we have no poise, or if the force is overwheling, skip the resist chance.
		...
		if( !(resist_roll == 1) ) //Did we succeed?
			...
			visible_message("<span class='attack'>[src] withstands the impact!</span>",
	...
	src.balance = max(src.balance - Ceiling(strength / 2),0) //High weapon stagger will deplete stamina faster.
	if(ishuman(src))
		var/mob/living/carbon/human/H = src
		H.update_balance_hud()
	if(src.balance <= 0)
//		balance = -1
		var/a = rand(1,3)
		switch(a)
			...
			if(2)
//				visible_message("<span class='attack'>[src] is smashed into the ground from \the [user]'s overwhelming force!</span>",
//				"<span class='danger'>You're smashed into the ground!</span>")
		...
		if(user)
			...
			if(!T.density)
				...
				visible_message("<span class='attack'>[src] slams into \the [T]!</span>",
				"<span class='danger'>You slam into \the [T]!</span>")
			*/