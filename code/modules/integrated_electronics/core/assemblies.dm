...
/obj/item/electronic_assembly
	...
	var/obj/item/stock_parts/cell/device/battery = null // Internal cell which most circuits need to work.
...
/obj/item/electronic_assembly/attackby(var/obj/item/I, var/mob/user)
	...
	else if(istype(I, /obj/item/stock_parts/cell/device))
		if(!opened)
			to_chat(user, "<span class='warning'>\The [src] isn't opened, so you can't put anything inside.  Try using a crowbar.</span>")
			return FALSE
		if(battery)
			...
			return FALSE
		var/obj/item/stock_parts/cell/device/cell = I
		user.drop_item(cell)
		cell.forceMove(src)
		battery = cell
		playsound(src, 'sound/items/Deconstruct.ogg', 50, 1)
...
/obj/item/electronic_assembly/proc/is_valid_tool(var/obj/item/I)
	return I.is_crowbar() || I.is_screwdriver() || istype(I, /obj/item/integrated_circuit) || istype(I, /obj/item/stock_parts/cell/device) || istype(I, /obj/item/integrated_electronics)
