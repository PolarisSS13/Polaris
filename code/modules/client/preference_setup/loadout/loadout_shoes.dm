...
/datum/gear/shoes/flats/selection
	display_name = "flats selection"
	path = /obj/item/clothing/shoes/flats

/datum/gear/shoes/flats/selection/New()
	..()
	var/list/flats = list()
	for(var/flat in typesof(/obj/item/clothing/shoes/flats))
		var/obj/item/clothing/shoes/flats/flat_type = flat
		flats[initial(flat_type.name)] = flat_type
	gear_tweaks += new/datum/gear_tweak/path(sortAssoc(flats))
