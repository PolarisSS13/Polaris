...
/obj/item/weapon/deck/attack_hand(mob/user as mob)
	...
	if(isstorage(src.loc) || src == H.r_store || src == H.l_store || src.loc == user) // so objects can be removed from storage containers or pockets. also added a catch-all, so if it's in the mob you'll pick it up.
		..()
