...
/client/verb/cmd_view_custom_event()
	...
	if(!custom_event_msg || custom_event_msg == "")
		to_chat(src, "<span class='filter_notice'>There currently is no known custom event taking place.")
		to_chat(src, "<span class='filter_notice'>Keep in mind: it is possible that an admin has not properly set this.")
		return

	to_chat(src, "<h1 class='filter_notice alert'>Custom Event</h1></span>")
