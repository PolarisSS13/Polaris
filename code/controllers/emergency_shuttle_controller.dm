	...
	if (!shuttle.location)	//at station
		if (autopilot)
			...
			if (evac)
				...
				priority_announcement.Announce(replacetext(replacetext(using_map.shuttle_docked_message, "%dock_name%", "[using_map.dock_name]"),  "%ETD%", "[estimated_time] minute\s")) //Polairs Edit
...
/datum/emergency_shuttle_controller/proc/call_transfer()
	...
	priority_announcement.Announce(replacetext(replacetext(using_map.shuttle_called_message, "%dock_name%", "[using_map.dock_name]"),  "%ETA%", "[estimated_time] minute\s")) //Polairs Edit
	atc.shift_ending()
