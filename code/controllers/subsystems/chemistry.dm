...
/datum/controller/subsystem/chemistry/proc/initialize_chemical_reactions()
	init_subtypes(/datum/chemical_reaction, chemical_reactions)

	for(var/reaction in chemical_reactions)
		var/datum/chemical_reaction/D = reaction
		if(D.required_reagents && D.required_reagents.len)
			var/reagent_id = D.required_reagents[1]

			var/list/add_to = instant_reactions_by_reagent // Default to instant reactions list, if something's gone wrong
//			if(istype(D, /datum/chemical_reaction/fusion)) // TODO: fusion reactions as chemical reactions
//				add_to = fusion_reactions_by_reagent
			if(istype(D, /datum/chemical_reaction/distilling))
