...
/datum/antagonist/proc/get_candidates(var/ghosts_only)
	...
	for(var/datum/mind/player in candidates)
		...
		else if(isDrone(player.current))
			candidates -= player
			log_debug("[key_name(player)] is not eligible to become a [role_text]: They are a repair drone. They have been removed from the draft.")
		else if(ispAI(player.current))
			candidates -= player
			log_debug("[key_name(player)] is not eligible to become a [role_text]: They are a personal AI. They have been removed from the draft.")
