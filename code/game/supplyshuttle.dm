...
/datum/controller/supply
	...
	var/points_per_diamond = 20 //20 points per sheet
	var/points_per_mhydrogen = 15
	var/points_per_uranium = 10
	var/points_per_platinum = 5
	var/points_per_phoron = 5
	var/points_per_osmium = 4
	var/points_per_gold = 4
	var/points_per_silver = 3
	var/points_per_deuterium = 2
	var/points_per_tritium = 1
	...
	proc/sell()
				...
				for(var/atom in CR)
					...
					if(istype(A, /obj/item/stack))
						...
						switch(P.get_material_name())
							if("diamond") diamond_count += P.get_amount()
							if("metallic hydrogen") mhydro_count += P.get_amount()
							if("uranium") uranium_count += P.get_amount()
							if("platinum") plat_count += P.get_amount()
							if("phoron") phoron_count += P.get_amount()
							if("osmium") osmium_count += P.get_amount()
							if("gold") gold_count += P.get_amount()
							if("silver") silver_count += P.get_amount()
							if("deuterium") deut_count += P.get_amount()
							if("tritium") tritium_count += P.get_amount()
		...
		if(diamond_count)
			points += diamond_count * points_per_diamond

		if(mhydro_count)
			points += mhydro_count * points_per_mhydrogen

		if(uranium_count)
			points += uranium_count * points_per_uranium
		...
		if(osmium_count)
			points += osmium_count * points_per_osmium

		if(gold_count)
			points += gold_count * points_per_gold

		if(silver_count)
			points += silver_count * points_per_silver

		if(deut_count)
			points += deut_count * points_per_deuterium

		if(tritium_count)
			points += tritium_count * points_per_tritium
