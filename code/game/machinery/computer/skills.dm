...
/*			if ("Search Fingerprints")
				...
				if ((!( t1 ) || usr.stat || !( authenticated ) || usr.restrained() || (!in_range(src, usr)) && (!isrobot(usr))))
					return
				active1 = null
				t1 = lowertext(t1)
				for(var/datum/data/record/R in data_core.general)
...
// Add comment
			if ("add_c")
				...
				if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!isrobot(usr))) || src.active1 != a1))
					return
				var/counter = 1
				while(src.active1.fields[text("com_[]", counter)])
					counter++
				...
				switch(href_list["field"])
					if("name")
						if (istype(active1, /datum/data/record))
							...
							if ((!( t1 ) || !length(trim(t1)) || !( authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!isrobot(usr)))) || active1 != a1)
								return
							active1.fields["name"] = t1
					if("id")
						if (istype(active1, /datum/data/record))
							var/t1 = sanitize(input("Please input id:", "Secure. records", active1.fields["id"], null)  as text)
							if ((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!isrobot(usr))) || active1 != a1))
								return
