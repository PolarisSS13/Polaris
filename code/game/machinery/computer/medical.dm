	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && isturf(src.loc))) || (isrobot(usr)))
		...
		if (src.authenticated)
				...
				switch(href_list["field"])
					...
					if("mi_dis")
						if (istype(src.active2, /datum/data/record))
							...
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!isrobot(usr))) || src.active2 != a2))
								return
			...
			if (href_list["search"])
				...
				if ((!( t1 ) || usr.stat || !( src.authenticated ) || usr.restrained() || ((!in_range(src, usr)) && (!isrobot(usr)))))
					return
				src.active1 = null
				src.active2 = null
				t1 = lowertext(t1)
