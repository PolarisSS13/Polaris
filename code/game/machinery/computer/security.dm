...
//RECORD DELETE
			...
			if ("Add Entry")
				...
				if ((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!isrobot(usr))) || active2 != a2))
					return
				var/counter = 1
				while(active2.fields[text("com_[]", counter)])
					counter++
...
/obj/machinery/computer/secure_data/proc/is_not_allowed(var/mob/user)
	return !src.authenticated || user.stat || user.restrained() || (!in_range(src, user) && (!isrobot(user)))
