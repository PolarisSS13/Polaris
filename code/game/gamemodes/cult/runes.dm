...
/obj/effect/rune/proc/sacrifice()
	...
	var/datum/bloodnet/GLOBnet = GLOB.blood_networks[CULT_BLOODNETWORK_GLOBAL]
	...
	if(culcount >= 5)
			...
			if(R.blood_DNA == src.blood_DNA)
				for(var/mob/living/M in orange(2,R))
					...
					if (R.invisibility>M.see_invisible)
						to_chat(M, "<span class='danger'>The air burns!</span>")
					else
		...
		for(var/obj/effect/decal/cleanable/blood/B in world)
			if(B.blood_DNA == src.blood_DNA)
				for(var/mob/living/M in range(1,B))
					M.take_overall_damage(0,5)
					to_chat(M, "<span class='danger'>\The [B] suddenly ignites, burning you!</span>")
...
/obj/effect/rune/proc/changeconstruct(var/mob/living/L as mob)
	if(istype(src,/obj/effect/rune))
		usr.say("N'ath reth sh'yro mas d[pick("'","`")]rarratheln!")
	else
		usr.whisper("N'ath reth sh'yro mas d[pick("'","`")]rarratheln!")

	if(!L || !istype(L, /mob/living/simple_mob/construct))
		L = locate(/mob/living/simple_mob/construct) in get_turf(src)

	var/mob/living/simple_mob/construct/A = L

	if(!A)
		return fizzle()

	A.visible_message("<span class='cult'>\The [A] vanishes into the air.</span>")

	var/turf/T = get_turf(A)

	to_chat(A, "<span class='cult'>Your form dissipates to the immaterial realms, and you feel a pulling sensation.</span>")
	A.forceMove(null)

	var/construct_class = input(A, "Please choose which type of construct you wish to become.") as null|anything in list("Juggernaut", "Wraith", "Artificer", "Harvester", "Cancel")

	if(construct_class)
		switch(construct_class)
			if("Juggernaut")
				var/mob/living/simple_mob/construct/juggernaut/Z = new /mob/living/simple_mob/construct/juggernaut (T)
				Z.key = A.key
				if(iscultist(usr))
					cult.add_antagonist(Z.mind)
				qdel(A)
				to_chat(Z, "<B>You are now playing a Juggernaut. Though slow, you can withstand extreme punishment, and rip apart enemies and walls alike.</B>")
				...
				qdel(src)
			if("Wraith")
				var/mob/living/simple_mob/construct/wraith/Z = new /mob/living/simple_mob/construct/wraith (T)
				Z.key = A.key
				if(iscultist(usr))
					cult.add_antagonist(Z.mind)
				qdel(A)
				to_chat(Z, "<B>You are now playing a Wraith. Though relatively fragile, you are fast, deadly, and even able to phase through walls.</B>")
				to_chat(Z, "<B>You are still bound to serve your creator, follow their orders and help them complete their goals at all costs.</B>")
				Z.cancel_camera()
				qdel(src)
			if("Artificer")
				var/mob/living/simple_mob/construct/artificer/Z = new /mob/living/simple_mob/construct/artificer (T)
				Z.key = A.key
				if(iscultist(usr))
					cult.add_antagonist(Z.mind)
				qdel(A)
				...
				qdel(src)
			if("Harvester")
				var/mob/living/simple_mob/construct/harvester/Z = new /mob/living/simple_mob/construct/harvester (T)
				Z.key = A.key
				if(iscultist(usr))
					cult.add_antagonist(Z.mind)
				qdel(A)
