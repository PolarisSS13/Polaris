...
/obj/item/weapon/technomancer_catalog/attack_self(mob/user)
	...
	switch(tab)
		...
		if(4) //Info
			...
			dat += "<br>"
			dat += "Each type of core has a unique special ability, of which the Equipment tab will tell you the specifics of each \
			one.  The abilities are generally very powerful, however they are also generally situational, and cores require time \
			for the ability to recharge.  The usage of the special ability does not cost any energy or instability, and cores \
			that are recharging the ability are not adversely affected while doing so, which means the core ability is limited \
			by cooldown time instead of energy or instability limitations, the opposite of functions."
			dat += "<br>"
