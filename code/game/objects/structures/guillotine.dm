...
/obj/structure/guillotine/attack_hand(mob/user)
	...
	switch(blade_status)
		...
		if(GUILLOTINE_BLADE_DROPPED)
			...
			sleep(GUILLOTINE_ANIMATION_LENGTH)
			raise_blade(user)
			return
			...
			if(has_buckled_mobs())
				if(user.a_intent == "harm")
					...
					if(do_after(user, GUILLOTINE_ACTIVATE_DELAY, target = src) && blade_status == GUILLOTINE_BLADE_RAISED)
						...
						sleep(GUILLOTINE_ANIMATION_LENGTH - 2)
						drop_blade(user)
					else
						current_action = 0
				else
...
/obj/structure/guillotine/unbuckle_mob(force = 0)
	if(buckled_mobs)
		buckled_mobs[1].pixel_y += GUILLOTINE_HEAD_OFFSET  // Move their body back
		buckled_mobs[1].layer -= GUILLOTINE_LAYER_DIFF
	. = ..()
