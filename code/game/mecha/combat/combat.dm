	...
	//			if (M.health <= 0) return
				...
				if(temp)
					...
					switch(damtype)
						...
						if("tox")
							if(H.reagents)
								if(H.reagents.get_reagent_amount(/datum/reagent/toxin/carpotoxin) + force < force*2)
									H.reagents.add_reagent(/datum/reagent/toxin/carpotoxin, force)
								if(H.reagents.get_reagent_amount(/datum/reagent/cryptobiolin) + force < force*2)
									H.reagents.add_reagent(/datum/reagent/cryptobiolin, force)
						else
							return
