...
/datum/unit_test/subsystem_atom_shall_have_no_bad_init_calls/start_test()
	if(SSatoms.bad_init_calls.len)
		log_bad(jointext(SSatoms.GenerateBadInitLog(), null))
...
/datum/unit_test/all_atoms_shall_be_initialized/start_test()
	...
	for(var/atom/atom in world)
		if(!(atom.atom_flags & ATOM_INITIALIZED) && !QDELETED(atom)) // Not ideal to skip over qdeleted atoms, but a lot of current code uses pre-init qdels
